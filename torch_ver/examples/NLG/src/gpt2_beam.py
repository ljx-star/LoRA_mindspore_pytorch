...
def save_predictions(args, all_predictions, step):
    pred_file = os.path.join(args.work_dir, f'{args.output_file.replace(".jsonl", "")}_step_{step}.jsonl')
    print(f'saving prediction file {pred_file}')
        ...
        for idx, data in enumerate(data_iter):
            ...
            if args.rank == 0:
                ...
                if (idx + 1) % args.save_interval == 0:
                    save_predictions(args, all_predictions, idx)

    if args.rank == 0:
        save_predictions(args, all_predictions, "final")
